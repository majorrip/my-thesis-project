...
proof-of-concept evaluation against a non-regularized baseline shows that
the variance constraint reliably prevents variance collapse (sustaining a
mean embedding standard deviation above the hinge threshold), while
revealing that at the current covariance weight the objective does not
prevent dimensional (rank) collapse---an outcome that motivates a targeted
regularizer-weight ablation. The architecture itself is compact
($\sim\!0.75$M parameters, $2.84$~MB) with millisecond CPU inference. A
full empirical validation on standard vision benchmarks is identified as
the primary item of future work.
